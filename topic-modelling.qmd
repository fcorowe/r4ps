...
We now have a data frame with authors, dates and comments. However, we need to create a Corpus style object to preserve both both the full text of our Reddit comments and the metadata to eventually move to a Document-Term matrix used Topic Modelling. We are going to be using the package `tidytext`.
...
We are going to use the DTM we created from the reddit data. We will use the `LDA()` function from the `topicmodels` package, setting k = 5, to create a five-topic LDA model. Almost any topic model in practice will use a larger k, but we will soon see that this analysis approach extends to a larger number of topics.
...
For this set of questions, we will be using the same dataset as in [Chapter 6](https://www.population-science.net/sentiment-analysis.html).

```{r}
twitter_df <- read.csv("./data/topic-modelling/uk_geo_tweets_24022021_01072022.csv")
```
...
5. BONUS QUESTION: What are the limitation of topic models on short texts, such as Tweets. 
...
twitter_df_clean <- twitter_df_clean %>% 
  sf::st_as_sf(coords = c(7,8))  %>%  # create pts from coordinates
...
Bail, C. 2020, Computational Social Science [Topic Modelling](https://sicss.io/2020/materials/day3-text-analysis/topic-modeling/rmarkdown/Topic_Modeling.html)

Silge, J. and Robinson, D, 2022. [Text Mining with R: A Tidy Approach](https://www.tidytextmining.com/)

[Release Memory in R](https://stackoverflow.com/questions/15561331/release-memory-in-r)
